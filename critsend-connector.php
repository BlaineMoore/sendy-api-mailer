...
class MxmConnect {
	...
	private static $hosts = array('http://mail1.messaging-master.com', 'http://mail4.messaging-master.com', 'http://mail5.messaging-master.com', 'http://mail9.messaging-master.com', 'http://mail25.messaging-master.com', 'http://mail29.messaging-master.com', 'http://mail13.messaging-master.com', );
	 ...
	 * Public Method
	 */
	public function __construct($user, $password, $fast = False, $internal = False) {
		$this -> fast = $fast;
		self::$user = $user;
		self::$key = $password;
	...
	public function sendCampaign(&$content, &$param_campaign, &$subscribers) {
		...
		//canonicanization
		if ($this -> fast == True) {
			$mxm = new MxmConnect(self::$user, self::$key, True, True);
		} else {
			$mxm = new MxmConnect(self::$user, self::$key, False, True);
		}
	...
	public function sendEmail(&$content, &$param_campaign, &$subscribers) {
		...
		foreach($subscribers as $subscriber) {
			$headers = array('From' => $param_campaign['mailfrom'], 'To' => $subscriber['email'], 'Subject' => $content['subject']);
			
			if (!empty($args['replyto'])) {
				$headers['replyto'] = $args['replyto'];
			}
	...
	private static function makeClient($host) {
		try {
			$client = new SoapClient($host . self::$wsdl, array('compression' => SOAP_COMPRESSION_ACCEPT | SOAP_COMPRESSION_GZIP, 'trace' => False, 'encoding' => self::$encoding));
		} catch (SoapFault $e) {
			return False;
		}
		return $client;
