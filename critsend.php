...
/*BM: New Content*/		
	                    ...
	                    $scheduling_name = '<mxm var="field4" />'; // (found tag is Name)
	                    $scheduling_email = '<mxm var="email" />'; // $email
	                    $scheduling_tags = array(); // custom fields
	                    $scheduling_tag_counter = 5; // numerical assignment for each custom field
...
/*BM: Matched Below */
						...
						{			
						    while($row2 = mysqli_fetch_array($r2))
						    {
								...
								{									
									//replace new links on HTML code
							    	$html_treated = str_replace('href="'.$link.'"', 'href="'.APP_PATH.'/l/'.$scheduling_short_subscriber_id.'/'.short($linkID).'/'.short($campaign_id).'"', $html_treated); /*BM: Updated line*/
							    	$html_treated = str_replace('href=\''.$link.'\'', 'href="'.APP_PATH.'/l/'.$scheduling_short_subscriber_id.'/'.short($linkID).'/'.short($campaign_id).'"', $html_treated); /*BM: Updated line*/
...
BM:Removed */
								$title_treated = str_replace($tag, $scheduling_name, $title_treated); /*BM:Added:*/	
							}
							else //if not 'Name', it's a custom field
							{
								if(!isset($scheduling_tags[$field])) { $scheduling_tags[$field] = $scheduling_tag_counter++; } /*BM: Added*/
								$scheduling_tag_fallbacks[$field] = $fallback;  /*BM: Added*/
								$title_treated = str_replace($tag, '<mxm var="field'.$scheduling_tags[$field].'" />', $title_treated);  /*BM: Added*/
/*BM:Removed								
...
BM:Removed */
								$html_treated = str_replace($tag, $scheduling_name, $html_treated); /*BM:Added:*/	
							}
							else //if not 'Name', it's a custom field
							{
								if(!isset($scheduling_tags[$field])) { $scheduling_tags[$field] = $scheduling_tag_counter++; } /*BM: Added */
								$scheduling_tag_fallbacks[$field] = $fallback; /*BM: Added */
								$html_treated = str_replace($tag, '<mxm var="field'.$scheduling_tags[$field].'" />', $html_treated); /*BM: Added */
/*BM:Removed								
								//if subscriber has no custom fields, use fallback
								if($custom_values=='')
									$html_treated = str_replace($tag, $fallback, $html_treated);
...
BM:Removed */
								$plain_treated = str_replace($tag, $scheduling_name, $plain_treated); /*BM:Added:*/	
							}
							else //if not 'Name', it's a custom field
							{
								if(!isset($scheduling_tags[$field])) { $scheduling_tags[$field] = $scheduling_tag_counter++; } /*BM: Added */
								$scheduling_tag_fallbacks[$field] = $fallback; /*BM: Added */
								$plain_treated = str_replace($tag, '<mxm var="field'.$scheduling_tags[$field].'" />', $plain_treated); /*BM: Added */
/*BM:Removed								
								//if subscriber has no custom fields, use fallback
								if($custom_values=='')
									$plain_treated = str_replace($tag, $fallback, $plain_treated);
...
BM:Removed */							
				    	...
				    	$html_treated = str_replace('<webversion', '<a href="'.APP_PATH.'/w/'.$scheduling_short_subscriber_id.'/'.$scheduling_short_subscriber_list.'/'.short($campaign_id).'" ', $html_treated); /*BM:Updated Line*/
				    	$html_treated = str_replace('</webversion>', '</a>', $html_treated);
				    	$html_treated = str_replace('[webversion]', APP_PATH.'/w/'.$scheduling_short_subscriber_id.'/'.$scheduling_short_subscriber_list.'/'.short($campaign_id), $html_treated); /*BM:Updated Line*/
...
"._('View report')." - $app_path/report?i=$app&c=$campaign_id";
					...
					$content = array('subject'=> $title_to_me, 'html'=> $message_to_me_html , 'text' => $message_to_me_plain); /*BM: Added */
					$scheduling_email_parameters['tag'] = array('sendy-automatic'); /*BM: Added */
					$emails = array(); /*BM: Added */
					$emails[0] = array('email'=>$from_email); //send email to 'From email' address that's used to send this campaign /*BM: Added */
					if($notify_campaign_sent) $emails[1] = array('email'=>$user_email);  //send email to main account owner /*BM: Added */
					scheduling_critsend_send_emails($content, $scheduling_email_parameters, $emails); /*BM: Added */
/*BM: Removed
					$mail2 = new PHPMailer();	
					if($s3_key!='' && $s3_secret!='')
					{
...
} // function scheduling_critsend($row) /*BM: Added */
?>
